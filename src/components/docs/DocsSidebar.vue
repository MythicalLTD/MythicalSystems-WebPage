<template>
    ...
    >
        ...
        <div class="h-full overflow-y-auto">
            <nav class="p-4">
                <div class="mb-6">
                    <h3 class="text-sm font-semibold text-purple-400 uppercase tracking-wider mb-3">Home</h3>
                    <ul class="space-y-2">
                        <li>
                            <router-link
                                to="/"
                                class="block px-3 py-2 rounded-lg text-gray-300 hover:text-white hover:bg-purple-500/20 transition-colors"
                                :class="{ 'bg-purple-500/20 text-white': isActive('/') }"
                                @click="$emit('close')"
                            >
                                Main Page
                            </router-link>
                        ...
                        <li>
                            <router-link
                                to="/docs"
                                class="block px-3 py-2 rounded-lg text-gray-300 hover:text-white hover:bg-purple-500/20 transition-colors"
                                :class="{ 'bg-purple-500/20 text-white': isActive('/docs') }"
                                @click="$emit('close')"
                            >
                                Main Documentation
                            </router-link>
                ...
                <div v-for="section in sections" :key="section.title" class="mb-6">
                    <h3
                        class="text-sm font-semibold text-purple-400 uppercase tracking-wider mb-3 cursor-pointer flex items-center justify-between"
                        @click="toggleSection(section.title)"
                    >
                        {{ section.title }}
                        <span
                            :class="{ 'rotate-180': isSectionActive(section) }"
                    ...
                    <ul v-show="isSectionActive(section)" class="space-y-2">
                        <li v-for="item in section.items" :key="item.title">
                            <router-link
                                :to="item.path"
                                class="block px-3 py-2 rounded-lg text-gray-300 hover:text-white hover:bg-purple-500/20 transition-colors"
                                :class="{ 'bg-purple-500/20 text-white': isActive(item.path) }"
                                @click="$emit('close')"
                            >
                                {{ item.title }}
                            </router-link>
