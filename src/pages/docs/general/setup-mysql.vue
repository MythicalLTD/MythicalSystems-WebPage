            ...
            <div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
                <div class="space-y-8">
                    ...
                    <Card title="Configuring MySQL">
                        <p class="mb-4">This section covers creating a MySQL user that has permission to create and modify users.</p>
                    </Card>

                    <Card title="Creating a user">
                        <p class="mb-4">Firstly we have to create a database user to be able to access and manage the server via any ip!</p>
                        <CodeBlock language="sql" :code="createUser" />
                    </Card>

                    <Card title="Assigning permissions">
                        <p class="mb-4">The command below will give your newly created user the ability to create additional users, as well as create and destroy databases.</p>
                        <CodeBlock language="sql" :code="assignPermissions" />
                    </Card>

                    <Card title="Allowing external database access">
                        <p class="mb-4">Chances are you'll need to allow external access to this MySQL instance in order to allow servers to connect to it. To do this, open my.cnf, which varies in location depending on your OS and how MySQL was installed. You can type <code class="bg-gray-800 text-white p-1 rounded">find /etc -iname my.cnf</code> to locate it.</p>
                        ...
                        <p class="mb-4">For MariaDB, the process is similar. Open the MariaDB configuration file, which is usually located at <code class="bg-gray-800 text-white p-1 rounded">/etc/mysql/mariadb.conf.d/50-server.cnf</code>. Add the following lines to the file:</p>
                        <CodeBlock language="ini" :code="mariadbConfig" /><br>
                        <p class="mb-4">Restart MariaDB to apply these changes. This will allow connections on all interfaces, enabling external connections. Ensure that the MariaDB port (default 3306) is allowed in your firewall.</p>
                    </Card>
                </div>
            </div>
        </div>
