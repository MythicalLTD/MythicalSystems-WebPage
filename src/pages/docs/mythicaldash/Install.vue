                ...
                <div class="space-y-8">
                    ...
                    <Card title="Important Note">
                        ...
                        <p class="mb-4">
                            ...
                            install it on
                            your panel if you have more than 5 users due to panel desync! Also, please do not modify any
                            data from
                            inside the panel due to the dash not being able to sync with the panel! Every change you
                            want to make,
                            please make it from inside the dash!
                        </p>
                    </Card>

                    <Card title="Picking a Server OS">
                        <p class="mb-4">MythicalDash runs on a small range of operating systems, so pick whichever you
                            are most
                            comfortable using.</p>
                                ...
                                <tr v-for="os in operatingSystems" :key="os.name + os.version">
                                    <td class="py-2">
                                        <span v-if="os.supported">
                                            <a :href="`/docs/mythicaldash/os/${os.name.toLowerCase()}`" class="text-blue-500 underline">{{ os.name }}</a>
                                        </span>
                    ...
                    <Card title="Installing Composer">
                        <p class="mb-4">Composer is a dependency manager for PHP that allows us to ship everything
                            you'll need code
                    ...
                    <Card title="Download Files">
                        <p class="mb-4">The first step in this process is to create the folder where the dashboard will
                            live and
                        ...
                        <p class="mt-4">Once it is downloaded you'll need to unpack the archive and then set the correct
                            permissions
                            on the core/ and tmp/ directories. These directories allow us to store files as well as keep
                            a speedy
                            cache available to reduce load times.</p>
                            <br>
                        <CodeBlock language="bash" :code="setPermissions" />
                    </Card>

                    <Card title="Composer Installation">
                        <p class="mb-4">After you've downloaded all of the files you will need to upgrade the core
                            components of the
                            client. To do this, simply run the commands below and follow any prompts.</p>
                        <CodeBlock language="bash" :code="composerInstall" />
                    </Card>

                    <Card title="Fixing MariaDB">
                        <p class="mb-4">Since MariaDB added a new setting in the latest version, you have to switch the
                            MariaDB UTF
                    ...
                    <Card title="Database Setup">
                        <p class="mb-4">Now that all of the files have been downloaded we need to configure some core
                            aspects of the
                            Dashboard. You will need a database setup and a user with the correct permissions created
                            for that
                            database before continuing any further.</p>
                        <CodeBlock language="sql" :code="databaseSetup" />
                    </Card>

                    <Card title="Environment Configuration">
                        <p class="mb-4">MythicalDash's core environment is easily configured using a few different CLI
                            commands
                    ...
                    <Card title="Crontab Configuration">
                        <p class="mb-4">Setting up cron jobs will be really important; this is not an optional step: the
                            cron jobs
                            will allow the dashboard to process data internally, and manage the queue system. First,
                            check if
                            crontab is installed by typing "crontab -e" in your terminal. Or, if you are using a hosting
                            service,
...
const operatingSystems = [
    ...
    { name: 'Ubuntu', version: '>=22.04', supported: true, notes: 'MariaDB can be installed without the repo setup script.' },
    { name: 'Debian', version: '11', supported: true, notes: 'It can run, but we recommend using 12 for long-term usage!' },
